...
def setup_module(m):
    ...
    compile_buffer_sizes('bziplz4', buffers)
    compile_buffer_sizes('bunziplz4', buffers)
    shell.run('make bsv csv blz4 bziplz4 bunziplz4', stream=True)
...
def test_props(args):
    ...
    assert result == run(csv, f'bsv.{buffer} | bunziplz4.{buffer} {prefix} >/dev/null && ls {prefix}_* | bziplz4.{buffer} {cols} | csv.{buffer}')

def test_selection():
    shell.run('echo -e "a\nb\n" | bsv | blz4 > a')
    shell.run('echo -e "1\n2\n" | bsv | blz4 > b')
    assert '1,a\n2,b' == shell.run('echo a b | bziplz4 2,1 | csv')
    assert 'a,1\nb,2' == shell.run('echo a b | bziplz4 1,2 | csv')
    assert 'a\nb' == shell.run('echo a b | bziplz4 1 | csv')
    assert '1\n2' == shell.run('echo a b | bziplz4 2 | csv')
    with pytest.raises(Exception):
        assert '1\n2' == shell.run('echo a b | bziplz4 0 | csv')
    with pytest.raises(Exception):
        assert '1\n2' == shell.run('echo a b | bziplz4 3 | csv')
    with pytest.raises(Exception):
        assert '1\n2' == shell.run('echo a b | bziplz4 1,1 | csv')

def test_different_lengths():
    shell.run('echo -e "a\nb\nc\n" | bsv | blz4 > a')
    shell.run('echo -e "a\nb\n" | bsv | blz4 > b')
    with pytest.raises(Exception):
        shell.run('echo a b | bziplz4')
