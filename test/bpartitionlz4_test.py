...
def setup_module(m):
    ...
    shell.run('make clean && make bsv csv blz4d bcatlz4 bpartitionlz4', stream=True)
...
def test_props(args):
    ...
    with shell.tempdir():
        ...
        assert stdout == shell.run(f'bsv | bpartitionlz4 {num_buckets} prefix', stdin=csv, echo=True)
        assert result == shell.run('bcatlz4 --prefix prefix*')
...
def test_without_prefix():
    with shell.tempdir():
        ...
        08
        """
        assert rm_whitespace(unindent(stdout)) == shell.run('bsv | bpartitionlz4 10', stdin=unindent(stdin))
...
def test_basic():
    with shell.tempdir():
        ...
        prefix_08
        """
        assert rm_whitespace(unindent(stdout)) == shell.run('bsv | bpartitionlz4 10 prefix', stdin=unindent(stdin))
        stdout = """
        prefix_04:e,f,g
        prefix_06:h,i,j
        prefix_08:b,c,d
        """
        assert unindent(stdout).strip() == shell.run('bcatlz4 --prefix prefix*')
        stdout = """
        prefix_04
        prefix_06
        prefix_08
